...
public class Minecraft implements Runnable, LevelLoaderListener {
	...
	private void handleMouseClick() {
		if (this.editMode == 0) {
			if (this.hitResult != null) {
				final Tile oldTile = Tile.tiles[Minecraft.level.getTile(
						this.hitResult.x, this.hitResult.y, this.hitResult.z)];
				final boolean changed = Minecraft.level
						.setTile(this.hitResult.x, this.hitResult.y,
								this.hitResult.z, 0);
				if (oldTile != null && changed) {
					oldTile.destroy(Minecraft.level, this.hitResult.x,
	...
	private void setupCamera(final float a) {
		GL11.glMatrixMode(5889);
		GL11.glLoadIdentity();
		GLU.gluPerspective(70.0f, this.width / (float) this.height, 0.05f,
				1024.0f);
		GL11.glMatrixMode(5888);
		GL11.glLoadIdentity();
	...
	private void pick(final float a) {
		...
		for (int i = 0; i < hits; ++i) {
			...
			this.hitResult = new HitResult(names[0], names[1], names[2],
					names[3], names[4]);
			if (bestResult == null
					|| this.hitResult.isCloserThan(this.player, bestResult,
							this.editMode)) {
	...
	public void render(final float a) {
		...
		if (this.hitResult != null) {
			...
			this.levelRenderer.renderHit(this.player, this.hitResult,
					this.editMode, this.paintTexture);
			this.levelRenderer.renderHitOutline(this.player, this.hitResult,
					this.editMode, this.paintTexture);
			GL11.glEnable(3008);
			GL11.glEnable(2896);
		}
			...
			while (this.running) {
				if (this.pause) {
					...
					while (System.currentTimeMillis() >= lastTime + 1000L) {
						this.fpsString = String.valueOf(frames) + " fps, "
	...
	private void drawGui(final float a) {
		...
		final int yMouse = screenHeight - Mouse.getY() * screenHeight
				/ this.height - 1;
		GL11.glClear(256);
		GL11.glMatrixMode(5889);
		GL11.glLoadIdentity();
		GL11.glOrtho(0.0, (double) screenWidth, (double) screenHeight, 0.0,
				100.0, 300.0);
		GL11.glMatrixMode(5888);
		GL11.glLoadIdentity();
	...
	private void setupFog(final int i) {
		final Tile currentTile = Tile.tiles[Minecraft.level.getTile(
				(int) this.player.x, (int) (this.player.y + 0.12f),
				(int) this.player.z)];
	...
	private FloatBuffer getBuffer(final float a, final float b, final float c,
			final float d) {
		this.lb.clear();
	...
	public void levelLoadUpdate(final String status) {
		...
		t.end();
		GL11.glEnable(3553);
		this.font.drawShadow(this.title,
				(screenWidth - this.font.width(this.title)) / 2,
				screenHeight / 2 - 4 - 8, 16777215);
		this.font.drawShadow(status,
				(screenWidth - this.font.width(status)) / 2,
				screenHeight / 2 - 4 + 4, 16777215);
		Display.update();
		try {
			Thread.sleep(200L);
		} catch (Exception ex) {
